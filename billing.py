...
class BillClass:
    def __init__(self,root,employee_name):
        self.root=root
        self.root.geometry("1370x700+0+0")
        self.root.title("Super Mart Inventory | Billing")
        ...
        #....clock....#
        self.lbl_clock=Label(self.root,text=f"Hi {self.employee_name}, Welcome to Super Mart Inventory \t\tDate: DD/MM/YYYY\t\t Time: HH:MM:SS",font=("times new roman",15),bg="#4d636d",fg="white")
        ...
        txt_contact=Entry(CustomerFrame,textvariable=self.var_contact,font=("times new roman",13),bg="lightyellow").place(x=370,y=35,width=150)
        ...
        Cal_Frame=Frame(Calc_Cart_Frame,bd=9,relief=RIDGE,bg="white")
        Cal_Frame.place(x=5,y=10,width=268,height=340)

        self.txt_cal_input=Entry(Cal_Frame,textvariable=self.var_cal_input,font=('arial',15,"bold"),width=22,bd=10,relief=GROOVE,justify=RIGHT)
        ...
        #===========Cart Details=========#
        CartFrame=Frame(Calc_Cart_Frame,bd=3,relief=RIDGE)
        CartFrame.place(x=280,y=8,width=245,height=342)
        self.CartTitle=Label(CartFrame,text="Cart \t Total Product: [0]",font=("goudy old style",13),bg="lightgrey")
        ...
        btn_clear_cart=Button(Add_CartWidgetsFrame,text="Clear",font=("times new roman",15,"bold"),bg="lightgrey",cursor="hand2",command=self.clear_cart).place(x=180,y=70,width=150,height=30)
        ...
        BTitle=Label(billframe,text="Customer Bill Area",font=("goudy old style",20,"bold"),bg="#ff3131",fg="white").pack(side=TOP,fill=X)
        ...
        btn_print=Button(billMenuFrame,text="Print",font=("goudy old style",15,"bold"),bg="lightgreen",cursor="hand2",command=self.print_bill)
        btn_print.place(x=2,y=80,width=120,height=50)

        btn_clear_all=Button(billMenuFrame,text="Clear All",font=("goudy old style",15,"bold"),bg="grey",fg="white",cursor="hand2",command=self.clear_all)
    ...
    def show(self):
        ...
        cur=con.cursor()
        try:
            cur.execute("select pid,name,price,qty,status from product where status is 'Active'")
            rows=cur.fetchall()
    ...
    def add_update_cart(self):
        ...
        elif self.var_qty.get()=="0" and int(len(self.cart_list))==0:
            messagebox.showerror("Error","Quantity should not be 0",parent=self.root)
            self.var_qty.set("1")
        elif int(self.var_qty.get())>int(self.var_stock.get()):
            messagebox.showerror("Error",f"Quantity out of stock\nMaximum you can buy {self.var_stock.get()}",parent=self.root)
            self.var_qty.set("1")
        else:
    ...
    def generate_bill(self):
        if self.var_cname.get()=="" or self.var_contact.get()=="":
            ...
            messagebox.showerror("Error","Please add product in cart",parent=self.root)
        else:
            op=messagebox.askyesno("Confirm","Are you sure to generate final bill")
            if op==True:
                #====Bill Top=====#
                self.bill_top()
                #====Bill Middle=====#
    ...
    def bill_top(self):
        ...
        bill_top_temp=f'''
\t    Super Mart Inventory
E-mail:contactus@smi.com    Phone-8620018912
Address: 42,Bandra Road,Mumbai-400050
{str("=")*47}
Customer Name: {self.var_cname.get()}
Ph no. :{self.var_contact.get()}
Bill No. {str(self.invoice)}\t\t\tDate: {str(time.strftime("%d/%m/20%y"))}
{str("=")*47}
Product Name\t\tPer Price\t Qty\tNet Price
{str("=")*47}
        '''
    ...
    def bill_middle(self):
        ...
        try:
            for row in self.cart_list:
                ...
                price=str(price)
                self.txt_bill_area.insert(END,"\n "+name+"\t   Rs."+price_per+""+"\t\t  "+row[3]+"    Rs."+price)
...
{str("=")*47}
    ...
    def update_date_time(self):
        ...
        self.lbl_clock.config(text=f"Hi {self.employee_name},Welcome to Super Mart Inventory \t\tDate: {str(date_)}\t\t Time: {str(time_)}")
    ...
    def logout(self):
        ...
        if op==True:
            messagebox.showinfo("Logout Successful",f"\nThanks {self.employee_name} for using our service")
            self.root.destroy()
...
if __name__=="__main__":
    ...
    obj=BillClass(root,employee_name)
    root.mainloop()